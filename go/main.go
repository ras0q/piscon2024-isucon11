...
func getIsuList(c echo.Context) error {
	...
	for _, isu := range isuList {
		...
		if foundLastCondition {
			if lastCondition.Condition == "UNKNOWN" {
				c.Logger().Error(err)
				return c.NoContent(http.StatusInternalServerError)
			}
...
func getIsuConditions(c echo.Context) error {
	...
	conditionLevels := strings.Split(conditionLevelCSV, ",")
...
func getTrend(c echo.Context) error {
	...
	err = db.Select(&lastCondition, "SELECT * FROM `isu_condition` WHERE `timestamp` IN (SELECT MAX(`timestamp`) FROM `isu_condition` GROUP BY `jia_isu_uuid`) ORDER BY `timestamp` DESC")
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	for _, character := range characterList {
		...
		for _, isu := range isuList {
			isuLastCondition, ok := lastConditionMap[isu.JIAIsuUUID]
			if !ok {
				continue
			}
